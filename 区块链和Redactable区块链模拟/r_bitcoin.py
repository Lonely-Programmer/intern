...
##            print()
    ...
    def sign(self,private_key,public_key,prev_outs,script_types,m_public_key,sub_public_key,M=None,N=None):
...
class Merkel:    #merkel tree
    class Coinbase(Tx):    #coinbase，继承Tx类
        def __init__(self,private_key,public_key,m_public_key,sub_public_key):    #指定矿工的私钥和公钥
            #metadata
            self.hash = zsign.random_hash(256)    #交易hash(id)
            self.ver = 1
            self.vin_sz = 2
            ...
            self.sign(private_key,public_key,[],['pay-to-public-key-hash'],m_public_key,sub_public_key)    #签名

        def sign(self,private_key,public_key,prev_outs,script_types,m_public_key,sub_public_key):    #签名，其中输入不需要签名
            for i in range(len(self.outputs)):
                self.outputs[i].script_type = script_types[i]
    ...
    def __init__(self,n,private_key,public_key,m_public_key,sub_public_key):   #需传入容量、矿工私钥、公钥
        self.data = [self.Coinbase(private_key,public_key,m_public_key,sub_public_key)]    #交易数据
        self.node = []    #hash树
        self.cstr = []
        self.n = n    #容量
    ...
    def calc_hash(self):    #更新整棵树的hash值
        ...
        for i in range(n): 
            tmp = str(self.data[i].h) + str(self.data[i].r) + zsign.to_str(self.data[i])
            if self.data[i].sigma != None and not(zsign.verify(tmp,self.data[i].sigma,self.data[i].pk)):
                print('MSIG!')
                pass
            ...
            if  self.data[i].sigma == None:
                tmp2.append('0' * 200)
                self.cstr.append('')
            elif chrs.HashCheck_CHRS(self.data[i].mpk, self.data[i].spk, self.data[i].h, self.data[i].r, self.data[i].arbitrary):
            ...
            standard_part = zsign.to_str(self.data[i],['h','r','sigma','spk','pk','mpk','arbitrary'])
            if self.data[i].sigma != None:
                ht, rt = Hash.Hash(RSA.importKey(self.data[i].pk), standard_part)
                ht = hex(ht)[2::]
                rt = hex(rt)
            else:
                ht = '0' * 200
...
class Block:    #单个区块
    def __init__(self,n,private_key,public_key,prev,m_public_key,sub_public_key):
        #需传入merkel tree容量、矿工私钥、公钥、前一个区块的hash值
        #self.coinbase = Tx()
        self.tree = Merkel(n,private_key,public_key,m_public_key,sub_public_key)    #merkel tree
...
class Chain:   #区块链
    def __init__(self,n,private_key,public_key,m_public_key,sub_public_key):
        self.dat = [Block(n,private_key,public_key,'0' * 64,m_public_key,sub_public_key)]    #所有区块，已初始化第一块
        self.n = n    #merkel tree容量
        self.private_key = private_key    #矿工私钥
        self.public_key = public_key    #矿工公钥
        self.m_public_key = m_public_key
        self.sub_public_key = sub_public_key
    ...
    def append(self,tx):    #增加交易
        ...
        if self.dat[-1].is_full():
            self.dat[-1].mine()
            self.dat.append(Block(self.n,self.private_key,self.public_key,self.dat[-1].hash,self.m_public_key,self.sub_public_key))
...
##        self.regulator_public_key = tmp[1]
        with open('msk.pem') as fin:
            self.regulator_private_key = RSA.importKey(fin.read())
        with open('mpk.pem') as fin:
            self.regulator_public_key = RSA.importKey(fin.read())
        with open('ssk.pem') as fin:
            self.sub_private_key = RSA.importKey(fin.read())
        with open('spk.pem') as fin:
            self.sub_public_key = RSA.importKey(fin.read())

        #设置矿工密钥
        tmp = zsign.random_key()
        self.miner_private_key = tmp[0]
        self.miner_public_key = tmp[1]
        self.chain = Chain(self.n,self.miner_private_key,self.miner_public_key,self.regulator_public_key,self.sub_public_key)
        ...
        for i in range(self.n-1):
            ...
            tmp.sign(self.user_private_key,self.user_public_key,[],['pay-to-public-key-hash'] * 2,self.regulator_public_key,self.sub_public_key)
            #tmp.set_output([(1000,[self.user_public_key,self.miner_public_key])])
            #tmp.sign([self.user_private_key,self.miner_private_key],[self.user_public_key,self.miner_public_key],[],['pay-to-script-hash'] * 2,self.regulator_public_key,self.sub_public_key,2,2)
    ...
    def generate_tx(self):    #生成新交易
        ...
        tx.sign(self.user_private_key,self.user_public_key,prev_outs,['pay-to-public-key-hash'] * 2,self.regulator_public_key,self.sub_public_key)
        #tx.sign([self.user_private_key,self.miner_private_key],[self.user_public_key,self.miner_public_key],prev_outs,['pay-to-public-key-hash','pay-to-script-hash'],self.regulator_public_key,self.sub_public_key,2,2)
