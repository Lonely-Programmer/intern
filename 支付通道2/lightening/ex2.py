...
class Data:    #网络结构
    def __init__(self,k,l,rs):    #初始化
        ...
        #复制用户
        account_num = [1] * self.user_num
        user_tmp = list(range(self.user_num))
        while len(user_tmp) < self.account_num:
        ...
        for i in range(len(idx)):
            account_raw += [idx[i]] * account_num[i]
        random.shuffle(account_raw)    #
        boundary = 0
        for i in range(0,len(account_raw)):
            if cnt < self.low_bound * len(self.dat[-2]):
                boundary = self.low_bound
            else:
                boundary = self.high_bound
            failure = 0
            while True:
                tmp = random.randint(0,len(self.dat[-2])-1)
                failure += 1
                #print(len(accounts[tmp]))
                if len(accounts[tmp]) < boundary and not(account_raw[i] in accounts[tmp]):
                    accounts[tmp].append(account_raw[i])
                    cnt += 1
                    break
                if failure > 2000 and i < len(account_raw)-1:
                    failure = 0
                    tmp = random.randint(i+1,len(account_raw)-1)
                    account_raw[i], account_raw[tmp] = account_raw[tmp], account_raw[i]
    ...
    def transaction(self,n,flag):
        ...
        for i in range(n):
            while True:
                ...
                if flag[0] == "data":
                    value = random.choice(self.trans_dat)
                elif flag[0] == "pareto":
...
class Demo:
    ...
    def average(self,k,l,rs,n,flag):
        ...
        for i in range(n):
            ...
            result = self.test.transaction(5000,flag)    #transaction number
            success.append(result[0] / 5000)    #transaction number
    ...
    def kl_test(self,flag=("data",)):
        ...
        for i in range(len(k)):
            ...
            for tmp_l in l[i]:
                for tmp_r in r:
                    print("k = ",tmp_k," depth = ",tmp_l-1," rs = ",tmp_r,"   ",self.average(tmp_k,tmp_l,tmp_r,10,flag))

    def rs_test(self,flag=("data",)):
        k = [6,5]
        l = [[4],[5]]
        r = [0,0.005,0.01,0.05,0.1]
        for i in range(len(k)):
            tmp_k = k[i]
            for tmp_l in l[i]:
                for tmp_r in r:
    ...
    def scalability_test(self,flag=("data",)):
        ...
        for i in range(len(k)):
            ...
            for tmp_l in l[i]:
                for tmp_r in r:
                    print("k = ",tmp_k," depth = ",tmp_l-1," rs = ",tmp_r,"   ",self.scalability(tmp_k,tmp_l,tmp_r,10,flag))
